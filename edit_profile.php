		...
		if($_POST['password']==$_POST['passverif'])
		{
			if(strlen($_POST['password'])>=6)
			{
				...
				{	$conn= new mysqli("localhost","root","","vithub");
					...
					{	$conn= new mysqli("localhost","root","","vithub");
						if(mysqli_query($conn,'update users set username="'.$username.'", password="'.$password.'", email="'.$email.'", avatar="'.$avatar.'" where id="'.mysqli_real_escape_string($conn,$_SESSION['userid']).'"'))
						{
							$form = false;
...
<?php
	...
	if($form)
	{
		...
		{	$conn= new mysqli("localhost","root","","vithub");
			$dnn = mysqli_fetch_array(mysqli_query($conn,'select username,email,avatar from users where username="'.$_SESSION['username'].'"'));
